...
static int DELUKS_endec_template(char *src, size_t srcLength,
...
int DELUKS_encrypt_to_storage(char *src, size_t srcLength,
			    const char *cipher,
			    ...
			    struct crypt_device *ctx)
{
	...
	if (r == -ENOTSUP || r == -ENOENT)
		return DELUKS_endec_template(src, srcLength, cipher, cipher_mode,
...
int DELUKS_decrypt_from_storage(char *dst, size_t dstLength,
			      const char *cipher,
			      const char *cipher_mode,
			      struct volume_key *vk,
			      unsigned int sector,
			      struct crypt_device *ctx)
{
	...
	/* Fallback to old temporary dmcrypt device */
	if (r == -ENOTSUP || r == -ENOENT)
		return DELUKS_endec_template(dst, dstLength, cipher, cipher_mode,
