/*
 * LUKS - Linux Unified Key Setup v2, "luks1" digest handler
 *
...
static int LUKS1_digest_verify(struct crypt_device *cd,
	int digest,
	...
	size_t volume_key_len)
{
	...
	if (!json_object_object_get_ex(jobj_digest, "hash_alg", &jobj1))
		return -EINVAL;
...
static int LUKS1_digest_store(struct crypt_device *cd,
	int digest,
	const char *volume_key,
	size_t volume_key_len)
{
	...
	log_dbg("Setting LUKS1 type key digest %d.", digest);
	...
	json_object_object_add(jobj_digest, "type", json_object_new_string("luks1"));
	json_object_object_add(jobj_digest, "keyslots", json_object_new_array());
	json_object_object_add(jobj_digest, "hash_alg", json_object_new_string(pbkdf.hash));
...
static int LUKS1_digest_dump(struct crypt_device *cd, int digest)
{
	json_object *jobj_digest, *jobj1;
	...
	json_object_object_get_ex(jobj_digest, "hash_alg", &jobj1);
...
const digest_handler LUKS1_digest = {
	.name   = "luks1",
	.verify = LUKS1_digest_verify,
	.store  = LUKS1_digest_store,
	.dump   = LUKS1_digest_dump,
};
