				     ...
				     size_t *hdr_json_size, int secondary)
{
	...
	version = be16_to_cpu(hdr->version);
	if (version != 2) {
		log_dbg("Unsupported LUKS2 header version %u.", version);
		return -EINVAL;
	}

	hdr_size = be64_to_cpu(hdr->hdr_size);
	if (hdr_size != LUKS2_HDR_16K_LEN) {
		log_dbg("Unsupported LUKS2 header size %zu.", hdr_size);
		return -EINVAL;
	}

	/* FIXME: sanity check checksum alg. */

	log_dbg("LUKS2 header version %u of size %zu bytes, checksum %s.",
		version, hdr_size, hdr->checksum_alg);
			 ...
			 char **json_area, uint64_t offset, int secondary)
{
	...
	r = hdr_disk_sanity_check_pre(hdr_disk, &hdr_json_size, secondary);
	if (r < 0) {
		close(devfd);
		return r;
	}
		   ...
		   const char *json_area, int secondary)
{
	...
	hdr_to_disk(hdr, &hdr_disk, secondary);
			...
			struct device *device, int do_recovery)
{
	...
	if (state_hdr1 != HDR_FAIL && state_hdr1 != HDR_FAIL_IO) {
		 ...
		 */
		r = hdr_read_disk(device, &hdr_disk2, &json_area2, LUKS2_HDR_16K_LEN, 1);
		if (r == 0) {
			jobj_hdr2 = parse_and_validate_json(json_area2, be64_to_cpu(hdr_disk2.hdr_size) - LUKS2_HDR_BIN_LEN);
			state_hdr2 = jobj_hdr2 ? HDR_OK : HDR_OBSOLETE;
		} else if (r == -EIO)
